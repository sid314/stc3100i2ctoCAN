...
extern I2C_HandleTypeDef hi2ci;
...
int I2c_CustomInit(void) { return STC31xx_I2C_OK; }
...
int STC3100_Write(unsigned char ByteCount, unsigned char RegisterAddr,
                  unsigned char *TxBuffer) {
  int retry;
  HAL_StatusTypeDef hal_status;

  for (retry = 0; retry < NBRETRY; retry++) {
    hal_status = HAL_I2C_Mem_Write(&hi2ci, STC3100_SLAVE_ADDRESS_8BIT,
                                   RegisterAddr, I2C_MEMADD_SIZE_8BIT, TxBuffer,
                                   ByteCount, I2C_TIMEOUT);
    if (hal_status == HAL_OK) {
      return STC31xx_I2C_OK;
    }
  }
  return STC31xx_I2C_ERROR;
}

/*******************************************************************************
 * Function Name  : STC3100_Read
 * Description    : utility function to read several bytes from STC3100
 * registers Return         : error status
 *******************************************************************************/

int STC3100_Read(unsigned char ByteCount, unsigned char RegisterAddr,
                 unsigned char *RxBuffer) {
  int retry;
  HAL_StatusTypeDef hal_status;
  if (RxBuffer == NULL) {
    return STC31xx_I2C_ERR_BUFFER;
  }
  for (retry = 0; retry < NBRETRY; retry++) {
    hal_status = HAL_I2C_Mem_Read(&hi2ci, STC3100_SLAVE_ADDRESS_8BIT,
                                  RegisterAddr, I2C_MEMADD_SIZE_8BIT, RxBuffer,
                                  ByteCount, I2C_TIMEOUT);
    if (hal_status == HAL_OK) {
      return STC31xx_I2C_OK;
    }
